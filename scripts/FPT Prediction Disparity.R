...
# Fair Policy Targeting (Prediction Disparity): Function
...
optimisePredictionDisparity = function(Y, X, D, S, m0, m1, ps, alpha = seq(from = 0.05, to = 0.95, length.out = N), 
  ...
  # the objective function is the prediction disparity, betas and u_j not in objective
  model$obj = c((1-S)/mean(1-S) - S/mean(S), rep(0, p+N))
  ...
  model$A = rbind(cbind(diag(1, nrow = n), -XX, matrix(0, nrow = n, ncol = N)), # policies - betas <= 1 
                  ...
                  c(rep(1, n), rep(0, p+N)), # capacity constraint
  ...
  # put bounds on the parameter space, for z_i and u_j  [0,1], for the betas [-1,1]
  model$ub =  c(rep(1, n+p+N))
  ...
  for (i in 1:N){
    ...
    model$quadcon[[i]]$Qc = matrix(0, nrow = n+p+N, ncol = n+p+N) # rows and cols must = ncol of A
    ...
    # insert the respective welfare (- to bring it to lhs)
  ...
  weight = 1-alpha[u == 1] # store the respective optimal weight alpha_j (1-alpha since alpha is the male weight)
...
# Estimation FPT Prediction Disparity
...
#### FPT Prediction Disparity Estimation
...
objective = apply(policies, 1, function(x) sum(x*(1-S)/mean(1-S)) - sum(x*S/mean(S)))
...
# start value
start_pred_disp = c(policies[minimal_obj,], pareto_beta[minimal_obj,], ifelse(c(1:N) == minimal_obj, 1, 0))

# optimise! after 10'000s gap of 3.72%
FPT_pred_disparity = optimisePredictionDisparity(Y, X = cbind(S, X), D, S, m0, m1, ps, alpha = alpha, tolerance = 1e-6,
                                                 W_bar = pareto_W_bar, capacity_constraint, start = start_pred_disp,
                                                 timelimit = 15000)


# save(list = c("FPT_pred_disparity"), file = "./results/fpt_disp.RData")
